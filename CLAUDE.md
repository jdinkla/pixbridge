...
- There is no default model and no config file. A model must always be specified explicitly — `--model` on the CLI (required on `generate`/`style-transfer`/`consistency-check`; commands print `--model is required` and exit 1 otherwise) or `model=` on the `ImageClient` methods. Enforcement is centralized in `BaseImageProvider.validate_params`, which raises `ValueError` when `model is None`. This is deliberate: model names churn, so no baked-in default is allowed to go stale. By default any model string flows through to the provider SDK — a provider only restricts models if its `ProviderCapabilities.supported_models` is a non-empty allowlist (OpenAI: `gpt-image-2`, GPT Image 2.5 Flare/Sunburst aliases and their published snapshots). When it is, `validate_params` rejects unknown models locally with a `ValueError` before any API call; otherwise (Gemini/xAI/vertex, `supported_models=None`) unknown models pass through and SDK-rejected combinations surface as runtime SDK errors.
- OpenAI supports `gpt-image-2`, `gpt-image-2.5-flare`, and `gpt-image-2.5-sunburst` (including the 2.5 `-2026-09-08` snapshots). Only 2.5 accepts `xhigh` and `max`; all accept `low`, `medium`, `high`, and `auto`. OpenAI support targets these families only (older `gpt-image-1.x` models are no longer supported; the `supported_models` allowlist enforces this). Its capability `sizes` list (`1024x1024`, `1024x1536`, `1536x1024`, `2560x1440`, `3840x2160`) is a set of recommended values for docs/autocompletion; actual validation is rule-based via `validate_openai_size` (both dims divisible by 16, ratio in `[1:3, 3:1]`, `max(W, H) ≤ 3840`, total pixels in `[655360, 8294400]`).
